...
detect_outbreaks <- function(ts, method = "average"){
  ...
  threshold <- qpois(0.95,mean)
  alarms <- pred_data$cases > threshold

  pred_data <- pred_data %>%
    mutate(expectation = mean,
               threshold = threshold,
               alarms = alarms)


  fit_data <- fit_data %>% mutate(expectation = model$fitted.values,
                      threshold = qpois(0.95, model$fitted.values),
                      alarms = NA_real_)
...
signals_average <- detect_outbreaks(ts0)
...
ts0_signals_4_weeks <- detect_outbreaks_multiple_weeks(ts0)
...
ts1_with_signals_average <- detect_outbreaks_multiple_weeks(ts1)
...
ts2_with_signals_average <- detect_outbreaks_multiple_weeks(ts2)
...
ts3_with_signals_average <- detect_outbreaks_multiple_weeks(ts3)
...
Help: You can use the function `create_season_data()` to add sine and cosine terms to your timeseries data before applying the new algorithm which can use these terms.
...
input_aggr <- data_linelist %>%
  mutate(week = isoweek(date_report),
         year = isoyear(date_report)) %>%
  summarise(cases = n(), .by = c(week,year))
```

### Task 4.1.2
Do you think we are done now? Do you have any thoughts what could be a problem here?
