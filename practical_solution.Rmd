...
plot_ts(ts0)
```
No specific patterns (no obvious trend, no seasonality) thus stationary time series. Some potential outbreaks at the end of the time series and in spring 2019.
...
```
Observations:
We see a constant average as computed expectation. The last time point is marked as outbreak which is expected as it was a peak compared to what was observed before in the time series. 
...
Increasing time trend and small outbreak at the end.
...
No it is not suited as it is not modeling the time trend. We would get a lot of false alarms just because of the increasing case numbers because of the time trend.

### Task 3.4
What are your ideas to improve this algorithm? Discuss in the group.

Add a time trend term to the regression.
...
Yes because now the time trend is modeled as well and when using the average a lot of time points in the past would have been labeled as outbreaks which are not real outbreaks.
...
Falling time trend. Outbreak at the end.
...
No it is not suited as it is not modeling the time trend. We are missing the outbreak at the end due to not modeling this.

### Task 3.4
What are your ideas to improve this algorithm? Discuss in the group.

Add a time trend to the model.
...
The R library SignalDetectionTool which was developed within the Joint Action United4Surveillance provides functions for you to simplify this process. We start again with the line list of cases and apply the earsC algorithm to it but we use functions from the tool.
...
As you can see we do not need to do add weeks and year to the data_line list, do not need to aggregate and transform to sts. All of this is done inside the functions `preprocess_data()` (adding of week and year and other preprocessing) and `get_signals()` (aggregating,transforming to sts if necessary,...) .
