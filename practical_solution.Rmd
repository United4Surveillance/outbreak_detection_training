...
Yes it has one more row. Week 53 in year 2020 was added to the aggregated data set with 0 cases.
...
In the process of aggregation age_group would need to be added to the variables inside .by to group by age_group as well. The completion of the aggregated time series would need to be done for each age group. For each age group time series we need to generate a separate sts object and then apply the earsC algorithm to each of them.
